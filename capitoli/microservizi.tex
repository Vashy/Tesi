...
Esistono vari tipi di architettura software, ma il fine di questa sezione è quello di discutere le differenze, i pro e i contro tra un'architettura tradizionale monolitica e la ben più recente architettura a \glsdisp{microservizio}{microservizi}\gloss.
...
Per aggiornare un singolo componente, è necessario fare il ri-\gls{deploy}\gloss\ dell'intera applicazione, comportando minuti, se non ore, di totale indisponibilità del
...
e progetti con target verso i \gls{big-data}\gloss.
Ecco perché negli ultimi anni il paradigma dei \glsdisp{microservizio}{microservizi}\gloss\ ha guadagnato
...
Un'architettura a \glsdisp{microservizio}{microservizi}\gloss\ può essere genericamente vista come
...
Qui il sistema non è composto da un unico grande monolite, ma viene suddiviso in molti componenti indipendenti, chiamati \glsdisp{microservizio}{microservizi}\gloss, poiché contenuti e di limitata responsabilità.
È buona prassi dare a ognuno di essi una singola responsabilità, ove possibile, aderendo al \gls{srp}\gloss.

Come si può vedere in figura \ref{fig:microservices-arch}, ogni \gls{microservizio}\gloss\ ha un suo database da cui poter attingere.

Per poter comunicare, ognuno di essi espone delle \gls{api}\gloss, comunemente via \acrshort{http}\gloss\ e opzionalmente viene sfruttata una coda di messaggi (e.g. \textit{Apache Kafka}) per sfruttare i vantaggi dell'asincronicità.
...
\paragraph*{Testabilità} Vista la natura distribuita dei \glsdisp{microservizio}{microservizi}\gloss, i \textit{test} di integrazione e di sistema possono risultare veramente complessi
...
\paragraph*{Complessità architetturale} I \glsdisp{microservizio}{microservizi}\gloss\ portano con sé complessità architetturale che in un monolite non è presente.
...
Implementare un'architettura a \glsdisp{microservizio}{microservizi}\gloss\ non è una pratica semplice: non è sufficiente dividere il sistema in tanti componenti disaccoppiati per poter dire che un sistema segue questo paradigma.
...
\begin{itemize}[noitemsep]
	\item una pagina web che utilizza le normali tecnologie del web, quali \acrshort{html} 5, \acrshort{css} e \gls{javascript}\gloss;
...
Come possono i vari consumatori dell'app sapere a quale \gls{microservizio}\gloss\ accedere per una determinata funzionalità?
...
\begin{itemize}
	\item esporre \gls{api}\gloss\ diverse, differenziate per tipo di client;
...
\paragraph*{Problema} Quando si dividono le funzionalità del \textit{business} in tanti piccoli frammenti logici (i \glsdisp{microservizio}{microservizi}\gloss), è necessario pensare
a come far collaborare i dati ottenuti da ognuno di essi.
Questa non è una responsabilità che può essere lasciata ai consumatori del servizio, anche perché li potrebbe portare a comprendere l'implementazione interna del prodotto, rompendo il principio di \gls{information-hiding}.
...
\begin{itemize}
	\item un \gls{microservizio}\gloss\ composto che fa le chiamate ai servizi necessari
...
\paragraph*{Problema} Ci sono alcuni concetti da tenere in considerazione quando si va a progettare un sistema a \glsdisp{microservizio}{microservizi}\gloss:
\begin{itemize}[noitemsep]
...
I dati persistenti di un servizio non possono essere acceduti direttamente da altri servizi, ma vanno ottenuti tramite le \gls{api}\gloss\ esposte dal servizio.
...
L'attività di ``scoperta'' dei \glsdisp{microservizio}{microservizi}\gloss\ è chiamata \textit{Service Discovery}. Ne sono presenti due tipi:
\begin{itemize}
