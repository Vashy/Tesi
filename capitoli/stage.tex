...
\end{itemize}
Il progetto di stage consiste nell'implementazione di una \gls{webapp} a \glsdisp{microservizio}{microservizi}\gloss\ per il trasferimento in una nuova piattaforma del servizio di \gls{skill-matrix} usato da Sync Lab per organizzare le competenze delle persone che vanno in azienda per un colloquio.
...
\begin{itemize}
	...
	\item il \textit{back-end} utilizzerà un'architettura a \glsdisp{microservizio}{microservizi}\gloss\ implementati con il \textit{framework} \gls{spring}\gloss (\gls{java}\gloss), per questioni di uniformità e semplicità, con \gls{mongodb}\gloss\ come \gls{dbms}\gloss.
\end{itemize}
...
Il mio ruolo nel progetto consisterà prettamente nell'implementazione di alcuni \glsdisp{microservizio}{microservizi}\gloss\ back-end e nel riadattamento del vecchio portale.
...
La seconda parte dello stage è invece incentrata all'effettiva implementazione dei \glsdisp{microservizio}{microservizi}\gloss\ del \textit{back-end} per il prodotto \textit{SyncRec}, il nuovo portale aziendale per la gestione delle competenze dei candidati.
...
I \glsdisp{microservizio}{microservizi}\gloss\ accordati da implementare sono riportati nel dettaglio in seguito. Ognuno di essi esporrà le \gls{api}\gloss\ tramite degli specifici \textit{endpoint}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \gls{java}\gloss\ Standard Edition - ripasso generale; % 24
		\item Introduzione al \gls{dbms}\gloss\ noSQL \gls{mongodb}\gloss. % 8
	\end{itemize}

	\item \textbf{Seconda Settimana}
	\begin{itemize}
		\item Approfondimenti \gls{mongodb}\gloss; % 8
